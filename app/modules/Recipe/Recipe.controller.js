...
export const getIngredientUsageReport = async (req, res) => {
  try {
    ...
    // 3. Consolidate sales data to find product IDs in bulk to avoid N+1 queries
    const consolidatedSales = new Map();
    const unresolvedProducts = [];

    const saleProductNames = aggregatedSales.map(sale => sale._id);
    const products = await Product.find({ productName: { $in: saleProductNames }, branch }, '_id productName').lean();
    const productMap = new Map(products.map(p => [p.productName, p._id]));

    for (const sale of aggregatedSales) {
      const productId = productMap.get(sale._id);
      if (productId) {
...
export const getDynamicPrice = async (req, res) => {
  try {
    ...
    // --- Step 3: Calculate the average cost for each ingredient in bulk ---
    const ingredientIds = recipe.ingredients.map(item => item.ingredientId);

    const avgPricesResult = await Purchase.aggregate([
      { $match: { purchaseDate: { $gte: startOfMonth, $lte: endOfMonth } } },
      { $unwind: "$items" },
      { $match: { "items.ingredient": { $in: ingredientIds } } },
      {
        $group: {
    ...
    const avgPriceMap = new Map(avgPricesResult.map(p => [p._id.toString(), p.averagePrice]));

    for (const item of recipe.ingredients) {
      const ingredientId = item.ingredientId;
      const averagePrice = avgPriceMap.get(ingredientId.toString());
...
export async function getProductsWithRecipeStatus(req, res) {
  ...
  try {
    ...
    // 6. Map and populate recipes for the paginated products in bulk to avoid N+1 queries
    ...
    const productsWithRecipes = products.map((product) => {
      ...
      return {
        productDetails: product.toObject ? product.toObject() : product,
        hasRecipe: !!recipe,
        recipe: recipe,
      };
    });
